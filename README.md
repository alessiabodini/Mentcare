...
2. <ins>Modifica della cartella clinica di un paziente</ins> (sempre da parte dello staff medico).  
   In questo caso, la figura incaricata, a partire dalla cartella clinica del paziente, può cliccare sul pulsante "Edit medical record". Da qui si apre una nuova pagina, da cui si può:
   - modificare le allergie del paziente scrivendo all'interno dell apposito campo;
   - modificare le patologie del paziente selezionando/deselezionando rispettivamente le nuove condizioni riscontrate nel paziente e quelle invece da cui risulta guarito.  
Una volta cliccato su "Save and continue" si aprirà poi una nuova pagine che permette di inserire eventuali nuovi sintomi per le patologie indicate. 
     

3. <ins>Modifica/cancellazione di farmaci e trattamenti</ins> precedentemente prescritti.  
   Per fare ciò basta cliccare sul link "Change" presente vicino al farmaco/trattamento d'interesse dalla cartella clinica del paziente. Questa azione riconduce a una nuova pagina, da cui è possibile:
     - modificare la dose (e l'unità) di un farmaco e similmente la descrizione e la frequenza di un trattamento (per una descrizione dettagliata vedere rispettivamente i casi di prescrizione di farmaci e di trattamenti), e poi cliccare sul tasto "Save" per salvare i cambiamenti;
     - cancellare tramite il pulsante "Delete" il farmaco o il trattamento in questione. Tale azione, come la precedente, riporta poi in automatico alla cartella clinica del paziente.
...
11. <ins>Cancellazione di un consulto</ins>  
...
![PatientRegistration](/uml/PatientRegistration.jpg)
